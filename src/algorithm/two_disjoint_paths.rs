...
pub fn two_disjoint_paths(graph: &UndirectedGraph, (s1, t1): (usize, usize), (s2, t2): (usize, usize)) -> Option<(Vec<usize>, Vec<usize>)> {
    let mut g = graph.clone();
    g.remove_edge(&(t1,t2));
    unsafe {
        g.add_directed_edge(t1,t2);
    }
    match shortest_bottleneck_path(&g, s1, s2, (t1,t2)) {
        Impossible => None,
        Possible {cost: _, mut path} => {
